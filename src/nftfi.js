...
export default {
  init: async function (options = {}) {
    ...
    const hasGnosisSafePks =
      options?.ethereum?.account?.multisig?.gnosis?.safe?.owners?.privateKeys;
    const hasGnosisSafeAddress =
      options?.ethereum?.account?.multisig?.gnosis?.safe?.address;
    ...
    if (
      (hasGnosisSafePks &&
        (hasWeb3Provider || hasAccountPk || hasEthersJsonRpcSigner)) ||
      (hasWeb3Provider &&
        (hasGnosisSafePks || hasAccountPk || hasEthersJsonRpcSigner)) ||
      (hasAccountPk &&
        (hasGnosisSafePks || hasWeb3Provider || hasEthersJsonRpcSigner)) ||
    ...
    const network = (await provider?.getNetwork()) || {
      chainId: options?.ethereum?.chain?.id,
    };
    ...
    if (options.ethereum?.account?.multisig?.gnosis) {
      ...
      if (
        ...
        ((providerAddresses.length > 0 &&
          providerAddresses.includes(options?.ethereum?.account?.address)) ||
          providerAddresses.length === 0)
      ) {
    ...
    const assertion =
      options?.dependencies?.assertion || new Assertion({ account, provider });
    ...
    const storage =
      ...
      new Storage({ storage: localStorage, config });
    const auth = new Auth({ http, account, config, utils, storage });
    const api =
      options?.dependencies?.api ||
      new Api({ config, auth, http, assertion, mutex });
    ...
    const loanFixedV2_1 =
      options?.dependencies?.loans?.fixed?.v2_1 ||
      new LoansFixedV2_1({ config, contractFactory });
    const loanFixedV2_3 =
      options?.dependencies?.loans?.fixed?.v2_3 ||
      new LoansFixedV2_3({ config, contractFactory });
    const loanFixedCollectionV2 =
      options?.dependencies?.loans?.fixed?.collection?.v2 ||
      new LoansFixedCollectionV2({ config, contractFactory });
    ...
    const loanFixedCollection = new LoansFixedCollection({
    ...
    const loansValidation = new LoansValidation({
    ...
    const loansCollectionOffer = new LoansCollectionOffer({
      v1: loansCollectionOfferV1,
    });
    const loansAssetOfferV1 =
      options?.dependencies?.loans?.assetOffer?.v1 ||
      new LoansAssetOfferV1({ config, contractFactory, ethers });
    ...
    const offersHelper = new OffersHelper({
      BN,
      Number,
      utils,
      offersSignatures,
    ...
    const offersValidator = new OffersValidator({
      erc20,
      ethers,
      config,
      contractFactory,
    });
    const offersRequests = new OffersRequests({
      api,
      account,
      config,
      result,
    ...
    const cryptoPunks = new CryptoPunks({
      config,
      utils,
      error,
      result,
    ...
    const immutables = new Immutables({
      config,
      account,
      error,
      result,
    ...
    const bundlesHelper = new BundlesHelper({
      config,
      contractFactory,
      ethers,
    });
    ...
    const allocationsOg = new RewardsOgAllocations({
      account,
      api,
      result,
      error,
    ...
    const rewardsEarn = new RewardsEarn({
